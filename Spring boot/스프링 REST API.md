...
- A는 B 가 통신 호출(communication calls)간 `상태(현재 및 누적된 프로세스 정보)`를 유지 한다고 가정하지 않음

- A는 B에 요청할 때마다 관련 `상태`의 `표현(representational)` 을 제공하고 이러한 동작 방식은 생존가능성과 회복탄력성을 향상시킨다.

- 이렇게 처리하는 이유는 통신문제가 발생하거나 B가 충돌해 재시작이 되어도, A와 상호작용한 `현재 상태`를 잃어버리지 않기 때문 → A가 다시 요청해 두 어플리케이션이 중단된 곳에서 통신을 이어간다.

**특징**

- 서버/클라이언트 구조 (**Client–server)**
    
    - 클라이언트와 서버는 서로 독립적이어야 하며, 클라이언트는 오직 URI 리소스만 알아야한다.
...
<img width="1000" alt="http://localhost:8080/api/article/3" src="https://github.com/user-attachments/assets/44eddbec-6010-429e-8b6b-dfb9964f7310">

<img width="1000" alt="http://localhost:8080/api/article" src="https://github.com/user-attachments/assets/70c9c398-8eda-47b0-8a3f-382ba9258d5f">
