...
![테스트-피라미드](https://velog.velcdn.com/images/alswn9938/post/bad1d374-0ab4-4c1e-9a8a-0291a5c83e60/image.png)

- 인수 테스트(UI) -> 통합 테스트 -> 단위 테스트
- 밑으로 내려 올 수록 더 많은 테스트 코드를 작성하라는 뜻으로 알려져 있음 (상위 테스트일수록 비용이 높기 때문에!)

- 단위 테스트 : 가장 작은 단위로 기능을 나누어 그 기능을 테스트하는 기법
- 통합 테스트 : 개발자가 변경할 수 없는 부분 (ex. 외부 라이브러리) 까지 묶어서 검증할 때 사용
- 인수 테스트 : 명세한 요구사항대로 잘 작동하는지 검증 / SW 내부 코드에 관심을 가지지 않는 블랙박스 테스트
...
- Oauth2.0 : 인터넷 사용자들이 비밀번호를 제공하지 않고, 다른 웹사이트 상의 자신들의 정보에 대해 웹사이트나 애플리케이션의 접근 권한을 부여할 수 있는 수단

### 개념증명 POC

- 새로운 프로젝트가 실제로 실현가능성이 있는지 효과와 효용, 기술적인 관점에서부터 검증을 하는 과정을 의미한다.
