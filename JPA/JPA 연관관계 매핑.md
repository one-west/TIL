...
2 단원은 회원 → 팀으로 접근하는 다대일 단방향 매핑이었다.

팀 → 회원으로 접근하는 관계를 추가하여 양방향 연관관계로 매핑
...
서로 다른 단방향 연관관계 2개를 잘 묶어서 양방향인 것처럼 보이게 할 뿐
...
- Member 테이블이 외래 키를 가지고 있으므로 [Member.team](http://Member.team) 의 주인이 된다.
...
JPA를 사용하지 않는 순수 객체의 경우 [Member.team](http://Member.team) 은 설정했지만, 반대의 연관관계를 설정하지 않았다.
...
위의 코드처럼 각각 호출하다 보면 실수로 둘 중 하나만 호출해서 양방향이 깨질 수 있다. 
